        ...
        if ( $cb_menu_icons != 'off' ) {
            ...
            if ( function_exists('login_with_ajax') ) {
                         if ( ( $cb_menu_icons == 'both' ) || ($cb_menu_icons == 'login' ) ) {
                              $cb_menu_output .= $cb_login_class . '<a href="#" title="' . esc_attr( $cb_login_title ) . '" class="cb-tip-bot" id="cb-lwa-trigger">' . $cb_login_space . '</a></li>';
                         }
            }
            if ( ( $cb_menu_icons == 'both') || ( $cb_menu_icons == 'search' ) ) {
                 $cb_menu_output .=  '<li class="cb-icon-search cb-menu-icon"><a href="#" title="' . __('Search', 'cubell') . '" class="cb-tip-bot" id="cb-s-trigger"><i class="fa fa-search"></i></a></li>';
            }
    ...
    function cb_breaking_news(){
        ...
        if ( $cb_breaking_source == 'cb_breaking_source_latest' ) {
            ...
            if ( $cb_breaking_filter == NULL ) { 
                ...
                $cb_breaking_cat = implode(",", $cb_breaking_filter); 
            }
            ...
            foreach( $cb_news_posts as $cb_post ) {
                ...
                $cb_output .= '<li><a href="' . get_permalink($cb_post["ID"]) . '" title="'. get_the_title( $cb_post_id ) . '" >' . get_the_title( $cb_post_id ) . '</a> </li> ';
            }
...
if ( ! function_exists( 'cb_byline' ) ) {
    function cb_byline( $cb_cat = true, $cb_post_id = NULL, $cb_short_comment_line = false, $cb_posts_on = false, $cb_post_views_off = false ) {
        ...
        if ( ( $cb_meta_onoff == 'on' ) || ( $cb_posts_on == true ) ) {
            $cb_byline = '<div class="cb-byline cb-font-header">' . $cb_author . $cb_date . $cb_cat_output . $cb_comments . $cb_post_views . '</div>';
        }
        ...
        if ( $cb_full_width_post == 'nosidebar' ) { $cb_number_related = $cb_related_posts_amount_full; } else { $cb_number_related = $cb_related_posts_amount; }
            ...
            if ( $cb_related_posts != NULL ) {
                ...
                foreach ( $cb_related_posts as $post ) {
                    ...
                    setup_postdata($post);
?>
                            <li class="cb-style-overlay cb-grid-entry cb-related-post no-<?php echo $i;?>">
    ...
    function cb_user_colors() {
        ...
        if ( is_category() ) {
            ...
            if ( $cb_background_image_cat != NULL ) {
                if ( isset(  $cb_background_image_cat['src'] ) ) {
                    $cb_background_image = $cb_background_image_cat['src'];
                } else {
                    $cb_background_image = $cb_background_image_cat['url'];
                }
            } else {
                $cb_background_image_setting = ot_get_option('cb_bg_image_setting', '1');
            }
            ...
            if ( ( $cb_background_image_single != NULL ) && ( ( $cb_featured_image_bg == 'standard' ) || ( $cb_featured_image_bg == 'standard-uncrop' ) || ( $cb_featured_image_bg == 'full-width' ) || ( $cb_featured_image_bg == 'off' )  ) ) {
        ...
        if ( is_category() ) {
            ...
            if ( function_exists('get_tax_meta') ) {
                ...
                if ( $cb_archive_title_bg != NULL ) {
                    ...
                    $cb_output[] = '<script type="text/javascript">jQuery(document).ready(function($){$("#cb-cat-header").backstretch("' . $cb_archive_title_bg_src[0] . '", {fade: 1200}); });</script>';
        ...
        if ( $cb_style == 'off' ) {
            ...
            if ( has_post_thumbnail() ) {
                ...
                if ( ( $cb_page_featured_style == '1' ) || ( $cb_page_featured_style == NULL ) ) {
                    ...
                    $cb_featured_image .= '<header id="cb-full-background-featured" class="cb-fis-big clearfix">';
            ...
            if ( $cb_featured_image_url != NULL ) {
                $cb_featured_image .= '<script type="text/javascript">jQuery(document).ready(function($){
                $("#cb-full-width-featured-img").backstretch("' . $cb_featured_image_url[0] . '", {speed: 350});
                });  </script>';
            }
            $cb_featured_image .= $cb_header;
            $cb_featured_image .= '<div id="cb-full-width-featured-img"></div></header>';
...
if ( ! function_exists( 'cb_get_body_classes' ) ) {
    function cb_get_body_classes() {
        ...
        return cb_get_sticky_option() . $cb_output . ' ' . ot_get_option( 'cb_gridslider_design', 'cb-gs-style-a' ) . ' ' . ot_get_option( 'cb_modal_style', 'cb-modal-dark' ) . cb_get_blog_style_full() . $cb_bs_color;
    }
}
...
if ( ! function_exists( 'cb_get_blog_style_full' ) ) {
    function cb_get_blog_style_full() {
        ...
        if ( function_exists( 'bbpress' ) && ( is_bbpress() ) ) {
            if  ( ot_get_option('cb_bbpress_sidebar', 'sidebar' ) == 'nosidebar' ) {
                $cb_check = true;
            }
        }
        ...
        if ( is_single() ) {
            global $post;
            if ( get_post_meta( $post->ID, 'cb_full_width_post', true ) == 'nosidebar' ) {
                $cb_check = true;
            }
        }

        if ( cb_is_woocommerce() ) {
            if ( ot_get_option('cb_woocommerce_sidebar', 'sidebar' ) == 'nosidebar' ) {
                $cb_check = true;
            }
        }
