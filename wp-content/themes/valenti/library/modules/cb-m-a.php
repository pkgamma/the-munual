 <?php /* Module: A */
	...
	$cb_qry = $cb_title_header = $cb_line_count_output = NULL;
    ...
    if ( $cb_qry->have_posts() ) {
        ...
        if ( $cb_line_count > 1 ) { $cb_line_count_output = 'cb-line-next'; } 
        ...
        if ( $cb_big_section == true ) {
            ...
            if ( $cb_line_wrapper  % 3 == 0 ) {
                echo '<div class="cb-line ' . $cb_line_count_output . ' clearfix">';
                $cb_closer = true;
...
?>
        <article <?php post_class('cb-article cb-color-hover clearfix no-' . $cb_count ); ?> role="article">
