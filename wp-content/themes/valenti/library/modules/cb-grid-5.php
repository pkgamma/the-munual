 <?php /* Grid of 5 Articles */
    ...
    if ( is_home() ) {
        ...
        $cb_title = $cb_module_style = NULL;
        $cb_qry = new WP_Query(array( 'posts_per_page' => 5, 'no_found_rows' => true, 'post_type' => $cb_cpt_output, 'tag_id' => $cb_cat_id, 'post_status' => 'publish', 'ignore_sticky_posts' => true )  );
    } else {
        $cb_qry = NULL;
    ...
    if ( $cb_qry->have_posts() ) : while ( $cb_qry->have_posts() ) : $cb_qry->the_post();
        ...
        if ( $i  == 1 ) {
             echo '<div class="cb-grid-block cb-style-overlay ' . $cb_module_style . ' clearfix">' . $cb_title_header . '<div class="cb-grid-5 cb-grid-module ' . $cb_flipped . 'clearfix">';
        }
?>
